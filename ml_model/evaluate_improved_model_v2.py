"""
Evaluation script for the improved V2 model
"""
import pandas as pd
import numpy as np
...
from improved_waste_prediction_model_v2 import ImprovedScientificWasteMLModelV2
...
def evaluate_model_performance(model, test_data, model_name):
    ...
    for _, row in test_data.iterrows():
        if model_name == "Pollution":
            ...
            actual = model.calculate_improved_global_warming(row)
    ...
    # Calculate percentage errors (avoid division by very small numbers)
...
def plot_predictions(pollution_results, gw_results, output_path='model_evaluation_v2.png'):
    """Creates visualization comparing predictions vs actual values."""
    fig, axes = plt.subplots(2, 2, figsize=(15, 12))
    fig.suptitle('Improved Model V2 Performance Evaluation', fontsize=16, fontweight='bold')
    ...
    max_val = max(gw_results['actuals'].max(), gw_results['predictions'].max())
...
def print_evaluation_report(pollution_results, gw_results, train_size, test_size):
    ...
    print("\n" + "="*80)
    print("🔬 IMPROVED MODEL V2 PERFORMANCE EVALUATION REPORT")
    print("="*80)
    print(f"\n📊 Dataset Information:")
...
def main():
    ...
    print("\n🚀 Starting Improved Model V2 Evaluation...")
    ...
    # Train the model
    print("\n🔄 Training improved V2 models...")
    model = ImprovedScientificWasteMLModelV2()
    ...
    plot_predictions(pollution_results, gw_results)
